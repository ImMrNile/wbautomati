...
exports.SPECIAL_EFFECTS = exports.TRANSPARENT_COLORS = exports.METALLIC_COLORS = exports.POPULAR_COLORS = exports.WB_COLORS = void 0;
exports.WB_COLORS = {
    // Основные цвета
    BASIC: [
    ...
    UTILS: {
        ...
        findByName: function (colorName) {
            var allColors = __spreadArrays(exports.WB_COLORS.BASIC, exports.WB_COLORS.EXTENDED);
            return allColors.find(function (color) {
                return color.value.toLowerCase().includes(colorName.toLowerCase());
            });
        },
        // Поиск цветов по категории
        findByCategory: function (category) {
            return exports.WB_COLORS.EXTENDED.filter(function (color) { return color.category === category; });
        },
        // Получение ближайшего цвета по RGB
        getClosestColor: function (targetRgb) {
            var allColors = __spreadArrays(exports.WB_COLORS.BASIC, exports.WB_COLORS.EXTENDED);
            var parseRgb = function (rgb) {
                var match = rgb.match(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i);
                return match ? {
            ...
            var targetColor = parseRgb(targetRgb);
            if (!targetColor)
                return null;
            var closestColor = allColors[0];
            var minDistance = Infinity;
            allColors.forEach(function (color) {
                var currentColor = parseRgb(color.rgb);
                if (currentColor) {
                    var distance = Math.sqrt(Math.pow(currentColor.r - targetColor.r, 2) +
        ...
        getAllColors: function () { return __spreadArrays(exports.WB_COLORS.BASIC, exports.WB_COLORS.EXTENDED); },
        // Группировка цветов по категориям
        getColorsByCategories: function () {
            var categories = {};
            exports.WB_COLORS.EXTENDED.forEach(function (color) {
                if (color.category) {
...
exports.TRANSPARENT_COLORS = [
    'прозрачный', 'полупрозрачный'
];
exports.SPECIAL_EFFECTS = [
    'матовый', 'глянцевый', 'перламутр', 'хамелеон', 'градиент'
];
exports["default"] = exports.WB_COLORS;
